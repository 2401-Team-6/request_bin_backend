...
async function createEndpoint(name) {
  ...
  await client.connect();
  const endpoint = await client.query('INSERT INTO endpoints (name) VALUES ($1) RETURNING *', [name]);
  console.log(endpoint.rows);
  // send back id and hash
  await client.end();
...
async function getRequests(id) {
  ...
  await client.connect();
  const response = await client.query('SELECT mongo_id, method, path, created FROM requests WHERE endpoint_id = $1', [id]);
